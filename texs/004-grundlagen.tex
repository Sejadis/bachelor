...
Die letzten beiden Mengen beschreiben den Effekt, den die Aktion nach ihrer Ausführung auf einen Zustand hat ($Post$), also Bedingungen, die wahr werden ($o_+$) und Bedingungen, die falsch werden ($o_-$).\\
...
Im linken Teil der Abbildung gibt es eine einfache Übersicht des Plans: welche Aktionen wurden wann ausgeführt und mit welchen Parametern.
Im rechten Teil der Abbildung wird dargestellt, welche Instanzen zu welchem Zeitpunkt als Teil welcher Aktion genutzt wurden.
\begin{figure}[ht!]
    \centering
...
Der in Listing~\ref{lst:ehc_algo} als Pseudocode beschriebene Algorithmus beginnt im Startzustand $M$.\\
...
M = I
while h(M) != 0 do
    breadth first Suche nach einem Zustand M' mit h(M') < h(M)
    if kein solcher Zustand gefunden then
        Fehlschlag zurückgeben, stoppen
    die Aktionen um zu M' zu kommen zum Ende des aktuellen Plans hinzufügen
    M = M'
\end{lstlisting}
\end{minipage}
Von diesem aus wird wird eine \emph{breadth-first} Suche gestartet.
Diese findet den nächstbesten Nachfolger, also den nächsten Zustand $M'$, dessen Heuristik besser als die des aktuellen Zustands ist.
Gelingt dies, wird der Weg von $M$ nach $M'$ zum Plan hinzugefügt.
...
Zustände werden in einer \emph{Queue} gespeichert und für eine Iteration der Suche wird der erste Zustand $M'$ von dieser entfernt und und die Heuristik berechnet.
Die Suche ist erfolgreich, wenn die Heuristik für diesen Zustand $M'$ besser als die von $M$ ist.
Andernfalls werden die Nachfolger von $M'$ zum Ende der \emph{Queue} hinzugefügt.
...
Ein \emph{dead end} für eine Planungsaufgabe $(O,I,G)$ ist definiert als ein Zustand $M$, der erreichbar ist und von diesem aus kein Plan das Ziel erreichen kann:\\
$\exists P: M = Result(I,P)$  und  $\neg\exists P': G\subseteq Result(M,P')$\\
...
- Visualisierungs bsp.\\
\subsection{Planung in \ac{ROS2}}\label{sec:ros2planning}
...
\begin{figure}
    ...
    \end{tikzpicture}
    \caption{\acs{BT} zur Ausführung einer spezifischen Aktion, nach~\cite{plansys}}
